...
|[01] |01.6 |27/04/2017 |DGE_V2_SFG_Spécifications_Fonctionnelles_Générales.docx |Spécifications fonctionnelles générales du DGE V2
|[02] | |{localdate} |SAF-architecture.pdf |Cadre d’architecture DGE V2, contenant notamment les noms des IS
|[03] |01.4 |27/04/2017 |DGE_V2_DSF_Dossier_de_Spécifications_Fonctionnelles.docx |Spécifications fonctionnelles détaillées du DGE V2
|[04] |01.0 |29/09/2016 |DGE_V2_FS_Diffuser_un_fichier_(DF).docx |Fiche Solution décrivant le routage pour la nature Diffusion Fichier pour le DGE V2
|[05] | |{localdate}|STS-Cache-Catalogue.pdf |Description du cache du catalogue DGE V2
|[06] |xxx |xxx |Ajouter FS DM Batch |
|[07] |xxx |xxx |Ajouter FS DM TR |
|[08] |xxx |xxx |DGE_V2_DSF_Liste_des_codes_d_erreurs.xlsx |Liste des codes d’erreurs DGE V2
|====================================================================================================================================================
...
Ce document décrit les opérations de routage réalisées par l’IS « MET » de l’application DGE V2 pour les natures d’échanges suivantes :
...
=== DgeBpmDf : routage Diffusion Fichier

Le routage de la nature Diffusion Fichier est réalisé par le package webMethods *DgeBpmDf* :

image:DgeBpmDf_package.png[image,width=162,height=71]image:DgeBpmDfTech_package.png[image,width=160,height=72]

==== Services publics

===== dgeBpmDf.pub:routeFile

====== Déclenchement

Le module de routage de la Diffusion Fichier se déclenche sur réception du document *dgeCommonsDocs.docs.df.bpm:StartBpmDf* publié par l’IS « B2B-FIC-PE-n », lorsque le résultat des contrôles d’entrée effectués sur le fichier est OK.

Ce document contient les données d’entête du fichier à router :

image::entete.png[image,width=150,height=133]

Sur réception de ce document, le trigger *dgeBpmDf.triggers:routeFile* est déclenché et le service associé démarre.


====== Traitement

Le service *dgeBpmDf.pub:routeFile* réalise le routage du fichier(cf. algorithmes fonctionnels : doc [04]) :
...
En fin de traitement, le service publie le document *dgeCommonsDocs.docs.df.bpm:EndBpmDf*, contenant les données nécessaires à la construction de l’ARLE et des entêtes de SORTIE :
...
=== DgeBpmDm : routage Diffusion Messages

Le routage de la nature Diffusion Messages est réalisé par le package webMethods *DgeBpmDm* :
...
===== dgeCommonsDocs.docs.dm.bpm:StartBpmDm

====== Déclenchement

Le module de routage de la Diffusion Messages se déclenche sur réception du document *dgeCommonsDocs.docs.dm.bpm:StartBpmDm* publié par l’IS B2B-FIC lorsque le résultat des contrôles d’entrée effectués sur la demande est OK. Ce document contient les informations du message à router.

_Cf. paragraphe "Documents WM" pour la description des documents webMethods._

Sur réception de ce document, le trigger *dgeBpmDm.triggers:startMsg* est déclenché et le service associé démarre.

====== Traitement

Le traitement est réalisé en quatre étapes correspondant aux services suivants :

* *dgeBpmDm.pub:startMsg*: appel à <<DmRoutingManager.startMsg,DmRoutingManager.startMsg>>
* *dgeBpmDm.pub:startRouteMsg*: appel à <<DmRoutingManager.startRouteMsg,DmRoutingManager.startRouteMsg>>
* *dgeBpmDm.pub:endRouteMsg*: appel à <<DmRoutingManager.endRouteMsg,DmRoutingManager.endRouteMsg>>
* *dgeBpmDm.pub:endMsg*: appel à <<DmRoutingManager.endMsg,DmRoutingManager.endMsg>>

Le schéma suivant représente les enchainements possibles de ces étapes :

image::schema2.png[image,width=631,height=326]

En fin de traitement, le service publie le document *dgeCommonsDocs.docs.dm.bpm:EndBpmDm*, signalant la fin de traitement d’un message. Ce document contient les informations nécessaires au connecteur DM pour créer le message de CRF ainsi que les messages de sortie à destination de chaque consommateur.
...
==== dgeCommonsDocs.docs.dm.bpm:Data (non publiable)
...
* dgeCommonsDocs.docs.dm.bpm:StartBpmDm
* dgeCommonsDocs.docs.dm.bpm:StartRouteMsg
* dgeCommonsDocs.docs.dm.bpm:EndRouteMsg
* dgeCommonsDocs.docs.dm.bpm:EndMsg
* dgeCommonsDocs.docs.dm.bpm:EndBpmDm
* dgeCommonsDocs.docs.dm.bpm:CallSNGI
* dgeCommonsDocs.docs.dm.bpm:CallDCR
...
* Entete :
** structure dgeCommonsDocs.docs.dm.bpm:Data

*Note : c'est « IdMessageTech » qui est utilisé pour retrouver le message en BDD (table MESSAGES).*

=== DgeBpmCommons

Les services webMethods de routage sont regroupés dans le package *DgeBpmCommons*.

==== Services publics

===== dgeBpmCommons.sched:reinitCacheRFO
...
Les méthodes JAVA de routage sont regroupées dans le jar du package *DgeBpmCommons*.
...
* dgeConnectorRFO.pub:getAllRisquesRDomainesValides
