...
NOTE: To generate your password in Gerrit, right click on your username -> *Settings* -> *HTTP password* -> *Generate Password*
...
* The *auStash = true* will allow you to *pull* while having some currently unstaged files (by automatically doing a *git stash* before and a *git stash apply* after).
====
...
image::git-fetch_pull.png[{half-width}]
...
* On Commit message 
** Click on **amend** if you want to amend a commit already started (using Gerrit vocabulary, adding a **patchset** on a **change**)
add your message and click on *Add Change-Id* to add an ID for your commit (you add a new change ID only when your previous is validated by the tech lead on Gerrit)
...
image::avoid-merge-conflict.png[{half-width}]
...
image::git-rebase.png[{full-width}]
...
TIP: If you don't have the option *fetch from Gerrit* : Go to the *Git repositories* view, under your repo, in *Remotes*, right click on *origin* and select *Gerrit configuration...*. Check that everything is OK and click *Finish*. Now you should have the option.
